...
class NoteManage(QtWidgets.QDialog):
    ...
    def connect_ui_events(self):
        ...
        :return:
        """
        self.ui.export_note_btn.clicked.connect(self.handle_export_note_click)
    ...
    def handle_export_note_click(self):
        checked_rows = self.note_model.checked_rows
        now = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
        desktop_path = QtCore.QStandardPaths.writableLocation(
            QtCore.QStandardPaths.StandardLocation.DesktopLocation)
        filepath = Path(desktop_path) / f'{now}.toml'
        save_file, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "导出笔记", str(filepath), "笔记集 (*.toml)")
        if save_file:
            data = {'core': {}}
            if not checked_rows:
                authors = set([note_proxy.note.author for note_proxy in self.note_model.proxy])
                ...
                for row in checked_rows:
                    try:
                        note_proxy = self.note_model.proxy[row]
                        author = note_proxy.note.author
                        author_dict = author.to_dict()
                        if author_dict not in authors_dict:
