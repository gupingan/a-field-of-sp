...
from app.lib.time import get_current_date
...
class UserProtocol(QtWidgets.QDialog):
    ...
    def build_interface(self):
        name = settings.value('gpa-khs-accept-name', '')
        date = settings.value('gpa-khs-accept-date')
        if not date:  # fixed: 修复某些情况下签署日期不显示的问题
            settings.setValue('gpa-khs-accept-date', get_current_date().strftime('%Y-%m-%d'))
            date = settings.value('gpa-khs-accept-date')
    ...
    def accept_protocol(self):
        ...
        if name:
            if validate_chinese_name(name):
                settings.setValue('gpa-khs-accept-protocol', 1)
                settings.setValue('gpa-khs-accept-name', name)
                settings.setValue('gpa-khs-accept-date', self.ui.label_date.text())
                self.close()
            else:
    ...
    def reject_protocol(self):
        settings.setValue('gpa-khs-accept-protocol', 0)
        settings.setValue('gpa-khs-accept-name', '')
        settings.setValue('gpa-khs-accept-date', '')
        self.close()

    def closeEvent(self, event):
        if settings.value('gpa-khs-accept-protocol', 0) == 0:
