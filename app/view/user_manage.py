...
from app.lib.browser import run_browser_in_thread, make_strategy
...
class CommentProxy:
    def __init__(self, user: PrivateUser, index: QtCore.QModelIndex, note_id: str, xsec_token: str):
        self.user = user
        self.index = index
...
class UserManage(QtWidgets.QDialog):
    ...
    def connect_ui_events(self):
        ...
        self.ui.qrcode_create_user_btn.clicked.connect(self.handle_qrcode_create_user_click)
        self.ui.mobile_create_user_btn.clicked.connect(self.handle_mobile_create_user_click)
        self.ui.manual_create_user_btn.clicked.connect(self.handle_manual_create_user_click)
    ...
    def handle_check_shield_btn_click(self):
        ...
        check_note_xsec_token = TomlBase.target_note_xsec_token
        ...
        if not check_note_xsec_token:
            QtWidgets.QMessageBox.critical(
                self, '失败',
                '你必须设置检测笔记所对应的 Xsec_token（链接中存在）。\n点击 设置 > 检测笔记'
            )
            return None
        ...
        for row in select_rows:
            ...
            signals = self.post_comment_thead.start_task(
                xhs.API().set_cookies(user.string_cookies).comment_post,
                check_note_id, comment_text)
            comment_proxy = CommentProxy(user, index, check_note_id, check_note_xsec_token)
    ...
    def recur_check_comment_state(self, user: PrivateUser, note_id: str, comment_id: str, cursor: str = "",
                                  xsec_token: str = ''):
        response = xhs.API().set_cookies(user.string_cookies).show_comments(note_id, cursor, xsec_token=xsec_token)
        ...
        if 'comments' in response['data']:
            ...
            return self.recur_check_comment_state(user, note_id, comment_id, cursor, xsec_token)

    def handle_qrcode_create_user_click(self):
        """
        显示添加用户的弹窗
        :return:
        """
        add_user_dialog = create_user.QrcodeCreateUser(parent=self)
        add_user_dialog.created.connect(self.create_after_update_table)
        add_user_dialog.exec()

    def handle_mobile_create_user_click(self):
        add_user_dialog = create_user.MobileCreateUser(parent=self)
        add_user_dialog.created.connect(self.create_after_update_table)
        add_user_dialog.exec()

    def handle_manual_create_user_click(self):
        add_user_dialog = create_user.ManualCreateUser(parent=self)
        add_user_dialog.created.connect(self.create_after_update_table)
        add_user_dialog.exec()
    ...
    def show_context_menu(self, pos):
        ...
        train_user_action = menu.addAction('养号')
        train_user_action.triggered.connect(self.train_selected_user)
    ...
    def open_selected_user(self):
        if not self._check_browser_path():
            return None

        index = self.ui.tableView.currentIndex()
        if index.isValid():
            user: PrivateUser = PrivateUserCenter.find(index.row())
            run_browser_in_thread(user.dict_cookies)
    ...
    def train_selected_user(self):
        ...
        if index.isValid():
            user: PrivateUser = PrivateUserCenter.find(index.row())
            train_strategy = [make_strategy(p) for p in TomlBase.train_strategy]
            run_browser_in_thread(user.dict_cookies, True, train_strategy)
