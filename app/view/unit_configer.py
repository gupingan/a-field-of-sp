...
class TaskerListItem(QtWidgets.QWidget):
    ...
    def update_ui_elements(self, first: int, add: int):
        row = int(self.property('row_index'))
        value = first + add * (row - 1)
        if value > self.ui.spinBox.maximum():
            value = self.ui.spinBox.maximum()
        if value < self.ui.spinBox.minimum():
            value = self.ui.spinBox.minimum()
...
class UnitConfiger(QtWidgets.QDialog):
    ...
    sentValue = QtCore.Signal(int, int)
    ...
    def build_interface(self):
        ...
        for tasker in self.factory.tasks:
            ...
            tasker_widget.setProperty('row_index', row)
    ...
    def handle_distribute_btn_click(self):
        first = self.ui.distribute_count_sb.value()
        add = self.ui.add_count_sb.value()
        self.sentValue.emit(first, add)
