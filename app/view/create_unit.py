...
from app.lib.core import Config, PrivateUser, ConfigCenter, PrivateUserCenter
...
class ConfigTableModel(QtCore.QAbstractTableModel):
    def __init__(self, data: t.List[Config]):
        super().__init__()
        self.proxy = [ConfigProxy(c) for c in data]
        self.select_config: t.Optional[Config] = None
...
class UserTableModel(QtCore.QAbstractTableModel):
    def __init__(self, data: t.List[PrivateUser]):
        super().__init__()
...
class CreateUnit(QtWidgets.QDialog):
    ...
    def connect_ui_events(self):
        ...
        self.ui.qrcode_create_user_btn.clicked.connect(self.display_create_user_dialog)
